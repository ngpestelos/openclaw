...
async function createNativeImageScenarioHarness(options?: {
  ...
  const api = {
    ...
    createSession,
    readEffectiveTools,
    readSessionTranscriptSummary: vi.fn(async () => ({
      finalText: "Generated QA lighthouse image",
      successfulToolCallCounts: { image_generate: 1 },
    })),
    runAgentPrompt,
    fetchJson,
    runQaCli,
...
describe("native image generation scenario", () => {
  ...
  it.each([
    ...
    await expect(result).rejects.toThrow("expected exactly one generated-image delivery");
    await expect(result).rejects.toThrow("with bytes matching");
    await expect(result).rejects.toThrow("transcript=outbound:qa-operator");
  ...
  it("rejects duplicate generated-image deliveries", async () => {
    ...
    await expect(result).rejects.toThrow("expected exactly one generated-image delivery");
    await expect(result).rejects.toThrow("saw 2");
    await expect(result).rejects.toThrow("transcript=outbound:qa-operator");
    await expect(result).rejects.not.toThrow("DO_NOT_EXPOSE_RAW_TOOL_OUTPUT");
  });
});
