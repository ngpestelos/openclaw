...
import { decisionDisplay, presentResult } from "./activity-run-inspector-fixtures.ts";
...
function receiptPage(
  ...
  return {
    ...base,
    decisions,
    decisionDisplays: decisions.map((receipt) => decisionDisplay(receipt)),
...
describeControlUiE2e("Control UI durable Activity run inspector", () => {
  ...
  it("keeps generic receipt prose out of every Activity presentation surface", async () => {
    ...
    const generic = decisionReceipt({
      id: secrets[0]!,
      summary: secrets[1]!,
      outcome: "allowed",
      reasonCode: "self_asserted_trusted_decision",
      coverageState: "enforced",
      remediation: secrets[3]!,
    });
    generic.remediation[0]!.code = secrets[2]!;
    generic.source.owner = "operator_approvals";
    generic.enforcement.policyRefs = [secrets[4]!];
    generic.enforcement.grantRefs = [secrets[5]!];
    const response = receiptPage([generic]);
    response.decisionDisplays = [
      decisionDisplay(generic, { state: "unverified" }, "decision-fact:1"),
    ];
