...
class SkillsPage extends OpenClawLightDomElement {
  ...
  private ensureInitialData() {
    ...
    if (
      ...
      void this.runClawHubSearch(this.clawhubSearchQuery);
    }
  }
  ...
  private changeClawHubQuery(query: string) {
    ...
    this.clawhubSearchTimer = setTimeout(() => void this.runClawHubSearch(query), 300);
  }

  private async runClawHubSearch(query: string) {
    const normalizedQuery = query.trim();
    this.debouncedClawHubSearchQuery = normalizedQuery;
    if (!normalizedQuery || !this.gateway.connected || !this.gateway.client) {
      await this.clawhubSearchTask.run([null, ""]);
      if (this.debouncedClawHubSearchQuery === normalizedQuery) {
        this.syncClawHubIcons();
      }
      return;
    }
    await this.clawhubSearchTask.run([this.gateway.client, normalizedQuery]);
    if (
      this.debouncedClawHubSearchQuery === normalizedQuery &&
      this.clawhubSearchQuery.trim() === normalizedQuery
    ) {
      // Task-backed getters are not Lit properties, so synchronize at the
      // completed task boundary where the current result is authoritative.
      this.syncClawHubIcons();
    }
  }
